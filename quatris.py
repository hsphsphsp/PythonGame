...
def checkJoystickButton(controller):
    for i in range(controller.get_numbuttons()):
        if controller.get_button(i) == 1:
            return [i, 'DOWN']
        else:
            return [i, 'UP']

def checkJoystickHat(controller):
    for i in range(controller.get_numhats()):
        if controller.get_hat(i) == 1:
            return [i, 'DOWN']
        else:
            return [i, 'UP']
...
def drawStatus(score, level, life):
    ...
    coinSurf = BASICFONT.render('Coin: %s' % coin, True, TEXTCOLOR)
    coinRect = coinSurf.get_rect()
    coinRect.topleft = (WINDOWWIDTH - 150, 230)
    DISPLAYSURF.blit(coinSurf, coinRect)
